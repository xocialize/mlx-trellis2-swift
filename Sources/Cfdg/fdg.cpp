...
// ---- C ABI entry (replaces the upstream torch entry; body below is verbatim upstream) ----
extern "C" int64_t fdg_mesh_to_flexible_dual_grid(
    const float* vertices, int64_t n_vertices,
    const int32_t* faces, int64_t n_faces,
    const float voxel_size_in[3],
    const int32_t grid_range_in[6],
    float face_weight,
...
{
    (void)n_vertices;
    const int F = (int)n_faces;
    const float* v_ptr = vertices;
    const int* f_ptr = faces;
    const float* voxel_size_ptr = voxel_size_in;
    const int* grid_range_ptr = grid_range_in;
    clock_t start, end;
    ...
    const int64_t N = (int64_t)voxels.size();
    *out_coords = (int32_t*)malloc(sizeof(int32_t) * N * 3);
    *out_dual = (float*)malloc(sizeof(float) * N * 3);
    *out_intersected = (uint8_t*)malloc(sizeof(uint8_t) * N * 3);
    if (!*out_coords || !*out_dual || !*out_intersected) return -1;
    for (int64_t i = 0; i < N; ++i) {
        for (int a = 0; a < 3; ++a) {
            (*out_coords)[i * 3 + a] = voxels[i][a];
            (*out_dual)[i * 3 + a] = dual_vertices[i][a];
            (*out_intersected)[i * 3 + a] = intersected[i][a] ? 1 : 0;
        }
    }
    return N;
}

extern "C" void fdg_free(void* p) { free(p); }
