// WeightMaterializer.swift — first-run download of the package's declared weight sources (the
// auto-materialization contract: the PACKAGE materializes; the app only picks the models folder).
// Each missing source's repo is snapshot-downloaded into the engine ModelStore layout
// (`<root>/<org>/<name>/…`) via swift-huggingface's HubClient, with per-file progress forwarded to
// `WeightDownloadProgress` so the engine's PreparationMonitor surfaces `.downloading`.
//
// NB the consolidated TRELLIS.2 repo is GATED (auto-approval, DINOv3 redistribution terms):
// HubClient env-detects `HF_TOKEN`, which must belong to an account that accepted the terms.
...
enum WeightMaterializer {
    ...
    /// Download every `source` into `root` (ModelStore layout). Sources download sequentially so the
    /// reported fraction is monotonic: source i of n spans [i/n, (i+1)/n).
